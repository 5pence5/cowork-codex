...
Call `codex_start_review` with `mode: "adversarial"` and preserve any focus text. Use `wait_seconds` for foreground waits and omit it for background work. The review should challenge the implementation approach, design choices, tradeoffs, and assumptions. Reviews run with `read-only`. Preserve the job id, poll with `codex_job_status` when needed, and return final Codex output verbatim through `codex_job_result`.
