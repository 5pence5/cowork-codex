...
For non-trivial research, diagnosis, or implementation tasks, compose the prompt using the `codex-prompting` skill before calling `codex_start_task`. Keep the final prompt compact, block-structured, and scoped to one Codex job. For `--resume`, send only the delta instruction unless the task direction changed materially.
